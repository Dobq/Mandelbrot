...
Remark that if - for example - You want to draw the same image, but in n times higher resolution, You have to both decrease dist 2*(n-1) times and increase res n times.
...
2. for each of squares, check if it's center (i.e. whole square) is calculated yet, if not, then check if values is corners are equal, if yes, then check if values on the sides are equal, if so, fill square with this value (squa_inse), it's job of squa_chec;
